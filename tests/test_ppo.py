...
from backend.learning.ppo import ActorCritic, PPOConfig, PPOTrainer, RolloutBuffer


def test_actor_critic():
    model = ActorCritic(latent_dim=256, action_dim=3, hidden_dim=256)
    z = torch.randn(4, 256)
    dist, value = model(z)
...
def test_ppo_trainer_select_action():
    config = PPOConfig(latent_dim=64, action_dim=3, hidden_dim=64)
